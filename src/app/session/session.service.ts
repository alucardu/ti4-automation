  ...
  private subscribeToSession(): void {
    ...
    this.apollo
      ...
      .subscribe({
        next: ({ data }) => {
          if (
            data?.userJoinedSession.session.host!.id !==
            data!.userJoinedSession.user.id
          ) {
            this.notificationService.openSnackBar(
              `${data?.userJoinedSession.user.name} has joined your session`,
              notificationType.SUCCESS
            );
